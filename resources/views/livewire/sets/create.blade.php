        ...
        <form wire:submit.prevent="save">
            <div class="space-y-4">
                ...
                <div x-data="{ open: false }" class="relative">
                    ...
                    <x-texts.text-small class="font-bold mb-1">Selecciona prodcutos para tu set</x-texts.text-small>
                        ...
                        <ul x-show="open" x-transition
                            ...
                            @forelse($products as $product)
                                <li wire:click="selectProduct({{ $product->id_product }})"
                                    ...
                                    <div class="font-semibold text-black2">{{ $product->name }}</div>
                                    <div class="text-gray-600">Categoría: {{ $product->category }}</div>
                                    <div class="text-gray-600">Precio: ${{ number_format($product->price, 2) }}</div>
                                    @if ($product->supplier)
                                        <div class="text-gray-600">Proveedor: {{ $product->supplier->name }}</div>
                                    @endif
                                </li>
                            @empty
                    ...
                    <div class="mt-4">
                        ...
                        <ul class="space-y-2">
                            @foreach ($selected as $id)
                                ...
                                @endphp
                                @if ($product)
                                    <li class="p-2 bg-gray-100 rounded flex justify-between items-center hover:bg-light-blue">
